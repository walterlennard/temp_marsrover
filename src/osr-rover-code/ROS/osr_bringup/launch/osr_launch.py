...
#Selbst geschrieben
#Damit virtual environment verwendet wird
import sys

venv_path = '/home/rover/osr_ws/venv/bin/activate_this.py'
...
def generate_launch_description():
    ...
    ld.add_action(
        Node(
            ...
            emulate_tty=True,
            respawn=True,
            parameters=[{'centered_pulse_widths': [139, 141, 149, 138]}]  # pulse width where the corner motors are in their default position, see rover_bringup.md.
        )
    )
    ld.add_action(
        DeclareLaunchArgument('enable_odometry', default_value='false')
        ...
        Node(
            ...
            respawn=True,
             parameters=[
                ...
                {"axis_angular.pitch": 3},  # axis to use for in-place rotation
                {"scale_angular.yaw": 1.25},  # scale to apply to angular speed, in rad/s: scale_linear / min_radius(=0.45m)
		{"scale_angular.pitch": 1.0},  # scale to apply to angular speed, in rad/s: scale_linear / min_radius(=0.45m)
		{"scale_angular_turbo.pitch": 1.0},
